...
try:
    from .config import (
        ...
        CLASS_SCORE_SCALES,
        CONF_THRESH,
        DUPLICATE_BOX_MIN_AREA_RATIO,
        DUPLICATE_BOX_MIN_COVERING_BOXES,
        DUPLICATE_BOX_MIN_PARTIAL_COVERAGE,
        DUPLICATE_BOX_PAIR_CLASSES,
        DUPLICATE_BOX_PAIR_MAX_AREA_RATIO,
        DUPLICATE_BOX_PAIR_MAX_CENTER_DIST_RATIO,
        DUPLICATE_BOX_PAIR_MAX_SCORE_RATIO,
        DUPLICATE_BOX_PAIR_MIN_CANDIDATE_COVERAGE,
        DUPLICATE_BOX_PAIR_MIN_SMALLER_COVERAGE,
        DUPLICATE_BOX_SINGLE_COVERAGE,
        DUPLICATE_BOX_UNION_COVERAGE,
        IMG_SIZE,
...
def _intersection_rect_xyxy(a: torch.Tensor, b: torch.Tensor) -> Optional[Tuple[float, float, float, float]]:
    """Return the intersection rectangle between two boxes, if it exists."""
...
def _center_distance_ratio_xyxy(a: torch.Tensor, b: torch.Tensor) -> float:
    ...
    aw = max(0.0, float(a[2]) - float(a[0]))
    ah = max(0.0, float(a[3]) - float(a[1]))
    bw = max(0.0, float(b[2]) - float(b[0]))
    bh = max(0.0, float(b[3]) - float(b[1]))
    denom = max(min(math.hypot(aw, ah), math.hypot(bw, bh)), 1e-6)
    return float(math.hypot(acx - bcx, acy - bcy) / denom)


def _class_uses_pair_duplicate_suppression(cls_id: int) -> bool:
    """Return whether a class should use stricter shifted-duplicate filtering."""

    allowed = {str(c) for c in DUPLICATE_BOX_PAIR_CLASSES}
    if not allowed:
        return False
    if str(cls_id) in allowed:
        return True
    if 0 <= int(cls_id) < len(CLASS_NAMES):
        return str(CLASS_NAMES[int(cls_id)]) in allowed
    return False


def _is_lower_conf_pair_duplicate(
    candidate: torch.Tensor,
    candidate_score: float,
    candidate_area: float,
    kept_box: torch.Tensor,
    kept_score: float,
    kept_area: float,
    inter_area: float,
    cls_id: int,
) -> bool:
    """Catch same-object boxes that are shifted enough to evade IoU-NMS."""

    if not _class_uses_pair_duplicate_suppression(cls_id):
        return False
    if candidate_area <= 0.0 or kept_area <= 0.0 or inter_area <= 0.0:
        return False

    candidate_coverage = inter_area / max(candidate_area, 1e-6)
    smaller_coverage = inter_area / max(min(candidate_area, kept_area), 1e-6)
    score_ratio = float(candidate_score) / max(float(kept_score), 1e-6)
    area_ratio = candidate_area / max(kept_area, 1e-6)
    center_ratio = _center_distance_ratio_xyxy(candidate, kept_box)

    return bool(
        candidate_coverage >= float(DUPLICATE_BOX_PAIR_MIN_CANDIDATE_COVERAGE)
        and smaller_coverage >= float(DUPLICATE_BOX_PAIR_MIN_SMALLER_COVERAGE)
        and score_ratio <= float(DUPLICATE_BOX_PAIR_MAX_SCORE_RATIO)
        and area_ratio <= float(DUPLICATE_BOX_PAIR_MAX_AREA_RATIO)
        and center_ratio <= float(DUPLICATE_BOX_PAIR_MAX_CENTER_DIST_RATIO)
    )


def _rect_union_area(rects: Sequence[Tuple[float, float, float, float]]) -> float:
    """Compute exact union area for a small set of axis-aligned rectangles."""
    ...
    for xi in range(len(xs) - 1):
        x_left, x_right = xs[xi], xs[xi + 1]
        if x_right <= x_left:
            continue
        x_mid = 0.5 * (x_left + x_right)
        for yi in range(len(ys) - 1):
            y_top, y_bottom = ys[yi], ys[yi + 1]
            if y_bottom <= y_top:
                continue
            y_mid = 0.5 * (y_top + y_bottom)
            if any(r[0] <= x_mid <= r[2] and r[1] <= y_mid <= r[3] for r in rects):
...
def _suppress_same_class_contained(
    ...
    """Remove same-class boxes covered by stronger boxes or box groups."""
    ...
    for idx in order.tolist():
        ...
        covering_rects: List[Tuple[float, float, float, float]] = []
        ...
        for kept_idx in keep:
            ...
            rect = _intersection_rect_xyxy(candidate, kept_box)
            if rect is None:
                continue

            inter_area = max(0.0, rect[2] - rect[0]) * max(0.0, rect[3] - rect[1])
            if _is_lower_conf_pair_duplicate(
                candidate=candidate,
            ...
            coverage = inter_area / max(candidate_area, 1e-6)
            covering_rects.append(rect)
            if coverage > max_single_coverage:
                max_single_coverage = float(coverage)
                max_single_area = float(kept_area)
            if coverage >= float(DUPLICATE_BOX_MIN_PARTIAL_COVERAGE):
                partial_cover_count += 1

        if max_single_coverage >= float(DUPLICATE_BOX_SINGLE_COVERAGE):
            area_ratio = candidate_area / max(max_single_area, 1e-6)
            if area_ratio >= float(DUPLICATE_BOX_MIN_AREA_RATIO):
                drop = True

        if not drop and partial_cover_count >= int(DUPLICATE_BOX_MIN_COVERING_BOXES):
            union_coverage = _rect_union_area(covering_rects) / max(candidate_area, 1e-6)
            if union_coverage >= float(DUPLICATE_BOX_UNION_COVERAGE):
                drop = True
