...
def _box_area_int(bbox: Sequence[float]) -> float:
    """Compute area for one xyxy box."""

    return float(max(0.0, float(bbox[2]) - float(bbox[0])) * max(0.0, float(bbox[3]) - float(bbox[1])))


def _intersection_rect(a: Sequence[float], b: Sequence[float]) -> Optional[Tuple[float, float, float, float]]:
...
def _is_lower_conf_pair_duplicate_int(
    cls_name: str,
    candidate: Sequence[int],
    candidate_score: float,
    candidate_area: float,
    kept_box: Sequence[int],
    kept_score: float,
    kept_area: float,
    inter_area: float,
) -> bool:
    ...
    if str(cls_name) not in {str(c) for c in DUPLICATE_BOX_PAIR_CLASSES}:
        return False
    ...
    center_ratio = _center_distance_ratio(candidate, kept_box)
...
def _suppress_same_class_contained_int(boxes: List[dict]) -> List[dict]:
    ...
    for box in ordered:
        ...
        for kept_box in kept:
            ...
            kept_area = _box_area_int(kept_bbox)
            rect = _intersection_rect(bbox_i, kept_bbox)
            if rect is None:
                continue

            inter_area = max(0.0, rect[2] - rect[0]) * max(0.0, rect[3] - rect[1])
            if _is_lower_conf_pair_duplicate_int(
                cls_name=cls,
                candidate=bbox_i,
            ...
            coverage = inter_area / max(area, 1e-6)
